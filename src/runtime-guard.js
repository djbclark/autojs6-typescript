...
exports.verifyRhinoRuntime = verifyRhinoRuntime;
function readRhinoVersion() {
    try {
 ...
 */
function verifyRhinoRuntime(expected, onMismatch) {
    const actual = readRhinoVersion();
    if (actual === expected)
        return true;
    const info = { expected, actual };
    if (onMismatch) {
        onMismatch(info);
        return false;
    }
    const msg = "Rhino runtime mismatch: expected " +
