...
Usage:
    check_require_bindings.py <project-dir> [<project-dir> ...]
...
def check_project(project_dir: Path) -> bool:
    js_files = find_js_files(project_dir)
    print(f"=== {project_dir} ({len(js_files)} compiled .js files) ===")
    ok = True

    for_of_hits = check_for_of(js_files)
    if for_of_hits:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args:
        print("usage: check_require_bindings.py <project-dir> [<project-dir> ...]", file=sys.stderr)
        return 2
    ...
    for raw_dir in args:
        ...
        if not project_dir.is_dir():
            ...
            all_ok = False
            continue
        all_ok = check_project(project_dir) and all_ok
